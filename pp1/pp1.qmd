...
```{r}
bechdel <- bechdel %>% 
    mutate(budget_mil = budget_2013 / 1000000) %>%
    mutate(intgross_mil = intgross_2013 / 1000000)
...
ggplot(bechdel, aes(x = clean_test)) +
    geom_bar() +
    labs(title = "Bechdel Test Results for Movies",
...
no_talk <- bechdel %>%
    filter(clean_test == "notalk") %>%
    nrow()

no_talk / nrow(bechdel)
```

##### The majority of movies in the dataset did not pass the Bechdel test (44.76%). In the same vein, the majority of movies failed belong to the no talk category (28.56%).
...
```{r}
ggplot(bechdel, aes(y = intgross_mil)) +
    geom_boxplot() +
    labs(y = "Gross by all films in millions of 2013 dollars")
```

##### The box plot indicates a high disparity of intgross between movies. The majority, represented by the box, sits at under the 1250 million dollars (2013). On the other hand, there are many outliers that makes up to 3000 million dollars.
...
```{r}
mean(bechdel$intgross_mil, na.rm = TRUE)
median(bechdel$intgross_mil, na.rm = TRUE)
```

##### The mean exceeds the median because of the uneven distribution of gross. In this case, few outliers at the high end with extremely high gross pull the average up. Whereas the majority of films hover around the bottom with below average gross.
...
```{r}
bechdel %>%
  summarize(sd(intgross_mil,  na.rm = TRUE))

bechdel %>%
  summarize(sd(budget_mil))
```

##### The standard deviation for intgross is higher, indicating a higher variability in intgross compared to budget.
...
##### The response value is intgross_mil because it measures the success of the film after being produced on budget_mil. Films with higher budgets tend to have higher quality and better marketing, which implies a higher chance of success in the market.
...
```{r}
bechdel %>%
  ggplot(aes(x = budget_mil, y = intgross_mil)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Budget in millions of 2014 dollars", y = "Gross in millions of 2014 dollars")
```
...
##### The data and the derived linear regression curve suggest a positive correlation relationship between budget_mil and intgross_mil.
...
```{r}
movie_model <- lm(intgross_mil ~ budget_mil, bechdel)
movie_model %>%
  summary()
```
...
##### intgross_mil = 3.06 * budget_mil + 27.14
...
##### Budget_mil coefficient indicates that, on average, a film would gross three times its budget. To put it in persepctive, a movie with a 10 million dollars budget is predicted to gross 30 million dollars on average.

d. Does it make sense to interpret the intercept in this model? Explain.

##### Not in particular. The intercept is too significant in some cases (movies with low gross) and too insignificant in other cases (movies with thousands of millions in gross).
...
##### intgross_mil_pred = 3.06 * budget_mil + 27.14
##### intgross_mil_pred = 3.06 * 290 + 27.14
##### intgross_mil_pred = 914.54
...
##### residual = intgross_mil_true - intgross_mil_pred
##### residual = 3171.931 - 915.4517 
##### residual = 3171.931 - 915.4517 
##### residual = 2256.479

```{r}
intgross_mil_true <- bechdel %>%
  filter(title == "Titanic") %>%
  select(intgross_mil)

intgross_mil_pred <- predict(movie_model, newdata = data.frame(budget_mil = 290))

intgross_mil_true - intgross_mil_pred
```


c. Based on your calculation in part b, did the model over- or under-estimate *Titanic*'s gross?

##### Because the residual was positive, the model under-estimated Titanic's gross.
