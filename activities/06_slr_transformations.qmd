...
# Fit the model
homes
home_mod <- lm(Price ~ Living.Area, homes)
...
The coefficient for `Living.Area` represents the average cost per squared unit (113.12). The intercept implies that the minimum cost for homes is 13439.39.
...
> Price = m * Living.Area.Shifted + b

> Price = 113.12 * Living.Area.Shifted + 13439.39

e. Now check your answer to part d by fitting the model.

```{r}
# Fit a model of Price vs. Living.Area.Shifted
home_mod <- lm(Price ~ Living.Area.Shifted, homes)
...
college_mod <- lm(GradRate ~ AdmisRate, college)
college_mod %>%
  summary()
```
...
bigmac_mod <- lm(bigmac_mins ~ gross_annual_teacher_income, bigmac)
bigmac_mod %>%
  summary()
...
```{r}
bigmac %>%
  ggplot(aes(x = log_sal, y = bigmac_mins)) +
  geom_point()
```
...
```{r}
bigmac_mod <- lm(bigmac_mins ~ log_sal, bigmac)
bigmac_mod %>%
  summary()
```
